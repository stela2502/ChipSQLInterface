...
if [ "$FIRST" -eq 1 ]; then
    sleep 5
    PGPASSWORD=$(cat /etc/postgres_password.txt) createdb -h localhost -U postgres genome_db
    PGPASSWORD=$(cat /etc/postgres_password.txt) psql -h localhost -U postgres -d genome_db -f /etc/setup_db.sql
fi