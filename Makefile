...
DEFINITION_FILE := ChipSQLInterface.def
...
deploy:
	...
	@if [ ! -f $(MODULE_FILE) ]; then \
	    ...
	    mkdir -p $(DEPLOY_DIR)/bin; \
            ...
            sed -i 's/^VERSION=.*/VERSION=${VERSION}/' $(DEPLOY_DIR)/bin/ChipSQLInterface ; \
            sed -i 's/^VERSION=.*/VERSION=${VERSION}/' $(DEPLOY_DIR)/bin/start_from_gtf ; \
	fi
